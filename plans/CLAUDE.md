...
- **Font payload — deferred (2026-08-17).** Fonts ship at **80.7 KB** for three faces (Fraunces 35.8 + Plus Jakarta Sans 26.7 + Courier Prime 18.2), `latin` subset, weight axis only. Two levers were measured and neither is being pulled: dropping Fraunces to a **single static weight** saves ~18 KB on the one preloaded, render-blocking face, and **character-subsetting** to the ~75 glyphs the site actually renders saves ~26 KB across all three. Together they reach 39.2 KB, a 51% cut. _Decided: not worth it yet — revisit only if load times become a concern._ Reopening either means a between-waves edit to `eleventy.config.js`, and the static-weight route also changes `package.json` (`@fontsource-variable/fraunces` → `@fontsource/fraunces`); both are frozen (§5, `AGENTS.md`), so it is a coordinated change, not a one-liner. For page weight generally, §5.3's `/details` photo will outweigh all three faces combined and is the better first place to look.
